...
alias ipa='ip addr'
mkd(){
 export USAGE='USAGE: mkd ${SOME DIR}'
 if [ ! -z $1 ];then
  if [ "$1" == 'build' ];then
...
skel(){
 ...
 if [ -d /etc/skel/.bin/ ];then
  if [ ! -d ~/.bin/ ];then
   mkdir -vp ~/.bin/
   rsync --recursive --whole-file --human-readable --verbose --update /etc/skel/.bin/ ~/.bin/
  fi
...
etcskel(){
 ...
 case "$1" in
  ...
  r|remote)
   skel;errchk
   if [ ! -f "/etc/skel/.bashrc.bckp" ];then b "/etc/skel/.bashrc";fi
   wget -O "/etc/skel/.bashrc.aftermarket" https://raw.githubusercontent.com/j1nh0/linx/main/bashrc
   rm '/etc/skel/.bashrc'&&ln -s '/etc/skel/.bashrc.aftermarket' '/etc/skel/.bashrc'
...
#Build sol
bldsol(){
 export USAGE='USAGE: bldsol ${ i(nstaller) | n(ano) | d(efault) <--YOU PROBABLY WANT THIS ONE | f(ull) | r(epo) | me } ${ bleach || clean }'
 #Test priviledges
 testpriv
 case "$1" in
  i|installer)
  ...
  f|full)
   ...
   rootlock l;errchk
  ;;
  me)curl 'https://raw.githubusercontent.com/j1nh0/j1nh0/main/consumables/buildme.sh'|bash;;
  *)usage;;
 esac
 case "$2" in
  bleach)bleach;;
  ...
  cisstig)cisstig;;
  *)echo 'No bleach or clean or cisstig requested!';;
 esac
 #Exit
...
#Upon login
cd ~
if oneko -h &> /dev/null;then oneko -dog & fi
