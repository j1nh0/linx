...
#Build sol
bldsol(){
 export USAGE='USAGE: bldsol ${ l(ight) | d(efault) <-- YOU PROBABLY WANT THIS ONE | i(nstall) | r(epo) }    ${bleach || clean}'
 #Test priviledges
 testpriv
 case "$1" in
  l|light)
...
#   apt install rsync;errchk
  ...
  i|install)
   ...
   rootlock l;errchk
  ;;
  r|repo)curl https://raw.githubusercontent.com/j1nh0/j1nh0/main/script/buildme.sh|bash;;
  *)usage;;
 esac
 case "$2" in
  bleach)bleach;;
