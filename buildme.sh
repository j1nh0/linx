...
case "$1" in
 ...
 rcl)
  ...
  for FORMKDIR in\
   ...
   $SAMBA/$SHARE\
  ;do
   if [ ! -d /$FORMKDIR/ ];then mkdir -vp /$JFORMKDIRD/;fi
   if [ -d /$FORMKDIR/ ];then
  ...
  if [ -f /$ETC/$SYSTEMD/$SYSTEM/$HOSTNAMESERVICE ];then
   chmod $PERMROOTFIL /$ETC/$SYSTEMD/$SYSTEM/$HOSTNAMESERVICE
   chown $PERMROOTOWN /$ETC/$SYSTEMD/$SYSTEM/$HOSTNAMESERVICE
   systemctl restart /$ETC/$SYSTEMD/$SYSTEM/$HOSTNAMESERVICE
  fi
  #HOME
  if [ -d /home ];then
   cd /home
