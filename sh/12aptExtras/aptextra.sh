#aptextra.sh
~/.bin/aptextra)
 export USAGE='USAGE: aptextra ${ l(ibdvd-pkg) | m(ega) | q(emu) | u(nattended) }'
 case "$1" in
  l|libdvd-pkg)
   aptu
   for APTLIBDVDPKG in\
  ...
  m|mega)
   aptu
   for APTMEGA in 'megasync_5.2.1-2.1_amd64.deb' 'megacmd_1.7.0-2.1_amd64.deb' 'nautilus-megasync_5.2.0-1.1_amd64.deb';do
