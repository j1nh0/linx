...
~/.bin/update)
 ...
 case "$1" in
  as|adb-sync)cd /tmp/&&git clone https://github.com/google/adb-sync.git&&cp -Rfv adb-sync/* "$SKELBIN"&&rm -rfv adb-sync/;;
  ...
  rclocal)
   wget -O /etc/rc.local https://raw.githubusercontent.com/j1nh0/linx/main/rclocal&&chmod 755 /etc/rc.local
  ;;
  minimal)
   update as;errchk
   update pf;errchk
