 ...
 case "$1" in
  b|barebones)
   for APTBAREBONES in\
    ...
    'ca-certificates' 'cmake' 'cpio' 'crunch' 'cryptsetup' 'cubic' 'curl'\
    'deluge' 'dosfstools'\
    'efitools'\
    'ffmpeg' 'flatpak' 'flex'\
    ...
    'htop'\
    'inxi'\
    'libelf-dev' 'libfuse2' 'libgoogle-perftools4' 'libgoogle-perftools-dev' 'libncurses-dev' 'libreoffice' 'libssl-dev' 'libtcmalloc-minimal4' 'linux-source'\
    ...
    'veracrypt' 'vim' 'vlc'\
    'zip'\
   ;do apt install "$APTBAREBONES";done
  ;;
  r|repo)
